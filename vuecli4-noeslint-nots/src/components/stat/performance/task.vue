<template>
    <div class="taskContainer">
        <div class="leftLiquid">
            <unit-liquid :type="'person'" :data='data' :dom='dom'></unit-liquid>
        </div>
        ...
        <div class="rightStat">
            ...
            <div class="echartBox">
                <unit-line :list='list?list:[]' :dom='dom' type='staff'></unit-line>
            </div>
            <div class="boxTable">
...
export default {
    ...
    data () {
        return{
            ...
            ajaxIng:false,
            title:[
                {key:'staffName',name:'销售员',width:10},
                {key:'subCompanyName',name:'分子公司',width:10},
                {key:'depName',name:'所属部门',width:10},
                {key:'entryDate_',name:'入职日期',width:10},
                {key:'targetAmount_',name:'年签约任务额（万元）',width:10,title:this.showTitle},
                {key:'amount_',name:'累计签约额（万元）',width:10,title:this.showTitle},
                {key:'percent_',name:'签约任务完成比例',width:10},
                {key:'q1Amount_',name:'Q1-签约额（万元）',width:10,title:this.showTitle},
                {key:'q2Amount_',name:'Q2-签约额（万元）',width:10,title:this.showTitle},
                {key:'q3Amount_',name:'Q3-签约额（万元）',width:10,title:this.showTitle},
                {key:'q4Amount_',name:'Q4-签约额（万元）',width:10,title:this.showTitle},
            ],
            list:[],
            data:{},
            total:0,
    ...
    methods:{
        ...
        getList() {
            ...
            this.$api.service.stat.yearsignamountlist(param,res=>{
                this.loading=false;
                switch(res.code){
                    case '0':
                        this.total=res.data.total;
                        let list=res.data.list?res.data.list:[];
                        let teamPercent = (res.data.teamPercent===null?100:res.data.teamPercent);
                        let teamPercent_ = Number(teamPercent/100);
                        let teamAmount = res.data.teamAmount===null?null:this.$pubfunc.transformPrice(res.data.teamAmount,1);
                        list.map(v=>{
        ...
        exportStat() {
            ...
            this.$api.download.yearsignamountexport(param,res=>{
                this.$pubfunc.downloadFile(res,'销售人员业绩签约额报表-'+param.year+'.xlsx');
                this.ajaxIng=false;
                this.$store.commit('alertFn',{
                    type:'success',
                    text:'导出成功',
