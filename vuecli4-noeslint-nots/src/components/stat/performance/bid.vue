...
export default {
    ...
    return {
      ...
      titleList: {
        all: [
          ...
          { key: 'tenderResultTxt', name: '投标结果', mustwidth: 120 },
          { key: 'tenderAmount_', name: '中标金额（元）', mustwidth: 120 },
          { key: 'planSignDate_', name: '预计签约日期', mustwidth: 200 },
        ...
        system: [
          ...
          { key: 'depName', name: '所属部门',  mustwidth: 120 },
          { key: 'customerName', name: '客户名称',  mustwidth: 220 },
          { key: 'projAmount_', name: '项目金额（元）',  mustwidth: 200 },
          { key: 'tenderAmount_', name: '中标金额（元）',  mustwidth: 200 },
          { key: 'projTypeTxt', name: '项目类型',  mustwidth: 120 },
          { key: 'tenderTime_', name: '投标时间',  mustwidth: 200 },
          { key: 'tenderResultTxt', name: '投标结果',  mustwidth: 120 },
          { key: 'isSign', name: '是否签约',  mustwidth: 120 },
          { key: 'planSignDate_', name: '预计签约日期',  mustwidth: 200 },
          { key: 'contractStatusTxt', name: '合同状态',  mustwidth: 120 },
          { key: 'remark', name: '备注', width: 500 },
        ],
          ...
          case '0':
            ...
            list.map(v => {
              ...
              v.tenderTime_ = v.tenderTime
                ...
                : ''
              v.planSignDate_ = v.planSignDate
                ? this.$pubfunc.TimeTransform('YYYY年MM月DD日', v.planSignDate)
                : ''
            })
            this.list = list
            break
    ...
    tableOper(key, item) {
      switch (key) {
        ...
        case 'projName':
          ...
          this.$pubfunc.setSStorage('tag', 'bid')
  ...
  created() {
    ...
    let tag = this.$pubfunc.getSStorage('tag'),
      judge = false
    this.$pubfunc.judgeBtn('performance:tender:sum')
      ? (this.tab.list.push({ id: '1', name: '投标项目-汇总' }),
        tag === '投标项目-汇总' ? (judge = true) : null)
      : null
    this.$pubfunc.judgeBtn('performance:tender:sysinte')
      ? (this.tab.list.push({ id: '2', name: '投标项目-系统集成项目' }),
        tag === '投标项目-系统集成项目' ? (judge = true) : null)
      : null
    this.$pubfunc.judgeBtn('performance:tender:idc')
      ? (this.tab.list.push({ id: '3', name: '投标项目-IDC项目' }),
        tag === '投标项目-IDC项目' ? (judge = true) : null)
      : null
