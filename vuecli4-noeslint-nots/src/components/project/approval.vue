<template>
  <div class="formPage projectApproval">
    ...
    <div class="formMainCont">
      <ul class="formUl">
        ...
        <li class="small">
          ...
          <div
            v-if="role || name === 'projectView' || data.tenderProjName"
            class="popoverTxt"
          >
            <unit-popover
          ...
          <input
            class="smallInput"
            v-if="!role && name === 'projectEdit' && !data.tenderProjName"
            type="text"
        ...
        <li class="small" v-if="role || name === 'projectView'">
      ...
      <ul class="formUl">
        <li class="small" v-if="!role && name === 'projectEdit'">
          <span class="formTitle">立项原因：</span>
          <div class="popoverTxt">
            <unit-popover
              :content="emptyTxt(data.proposalReasonTxt)"
      ...
      <ul class="formUl">
        ...
        <li class="small">
          ...
          <div
            v-if="role || name === 'projectView' || data.companySupport"
            class="popoverTxt"
          >
            <unit-popover
          ...
          <input
            class="smallInput"
            type="text"
            v-if="!role && name === 'projectEdit' && !data.companySupport"
        ...
        <li
          class="small"
          v-if="(role || name === 'projectView') && Number(data.projStatus) > 3"
        >
        ...
        <li class="small marginBottom" v-if="role || name === 'projectView'">
          <span class="formTitle"
            ><i
              class="updatedetail"
              title="查看修改信息"
      ...
      <div class="formMainCont_top">
        <ul class="formUl clearfix">
          <li :class="{ small: name !== 'projectAdd' }">
            <span
              class="formTitle"
              style="display: inlin-block; height: 35px; line-height: 35px"
            >
              最终用户：
            </span>
            <el-input
              v-if="!role && name === 'projectEdit'"
              class="contactCon auto"
              maxlength="50"
              placeholder
              v-model="result.finalCustomer"
            ></el-input>
            <div class="popoverTxt lh25" v-if="role || name === 'projectView'">
              <unit-popover
        ...
        >
          <span class="formTitle"
            ><i
              class="must"
              v-if="
                !role && name === 'projectEdit' && data.commitConfirm !== '1'
              "
              >*</i
            ><i
              class="updatedetail"
          ...
          <div
            v-if="
              role || name === 'projectView' || !(data.commitConfirm !== '1')
            "
          >
            {{
              data.commitStatus
          ...
          <el-radio
            v-if="!role && name === 'projectEdit' && data.commitConfirm !== '1'"
    ...
    <div class="formMainTitle" v-if="!role && name === 'projectEdit'">
      <span>限制修改信息</span><a>（每项最多修改两次）</a>
    </div>
    <div class="formMainCont" v-if="!role && name === 'projectEdit'">
      <ul class="formUl">
        <li :class="{ errorBorder: error.key === 'projAmount_' }">
          <span class="formTitle"><i class="must">*</i>项目金额：</span>
          <div
            class="input is-disable moneyInput"
            v-if="!data.updateFlag.projAmount"
          >
            <unit-popover
              :content="
                emptyTxt($pubfunc.transformPrice(result.projAmount, true))
              "
            ></unit-popover>
          </div>
          <i v-if="!data.updateFlag.projAmount">元</i>
          <input
            class="moneyInput"
            v-if="data.updateFlag.projAmount"
            type="text"
        ...
        >
          ...
          >
          <div
            class="input is-disable textarea"
            v-if="!data.updateFlag.projAmount"
          >
            您已修改两次项目金额，无法再次修改
          </div>
      ...
      <ul class="formUl">
        ...
        <li>
          ...
          <div class="input is-disable" v-if="!data.updateFlag.mainBrand">
            <unit-popover :content="emptyTxt(result.mainBrand)"></unit-popover>
          </div>
          <input
            type="text"
...
export default {
  ...
  methods: {
    ...
    getDropData() {
      ...
      // console.log(this.data, this.role)
      if (!this.role) {
      ...
      if (this.data.staffRole === "preSale") {
        ...
        if (this.data.tecDirector === this.userInfo.staffId) {
    ...
    submit(cb) {
      ...
      } else if (this.data.owner === this.data.userInfo.staffId) {
        // 项目金额
