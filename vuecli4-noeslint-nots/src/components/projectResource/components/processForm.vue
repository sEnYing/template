<template>
  <div class="processForm">
    ...
    <div class="formbox">
      ...
      <div class="four">
        <span>项目名称</span>
        <span :title="data.projName">{{ data.projName }}</span>
        <span>项目编号</span>
        <span>{{ data.projNo }}</span>
      </div>
      <div class="four">
        <span>客户名称</span>
        <span>{{ data.customerName }}</span>
        <span>项目金额</span>
        <span>{{ data.projAmount }}（元）</span>
      </div>
      <div class="four">
        <span>项目类型</span>
        <span>{{ data.projTypeTxt }}</span>
        <span>项目赢率</span>
        <span>{{ data.winRateTxt }}</span>
      </div>
      <div class="two">
        <span style="height: 120px; line-height: 120px"
          ><span class="require">*</span>项目背景及内容描述</span
        >
        <span
          style="height: 120px"
          v-if="type !== 'add'"
          v-html="processDetail.projDesc"
        ></span>
        <span
          style="
            height: 120px;
      ...
      <div class="two">
        <span>售前工程师</span>
        <span v-if="type !== 'add'">{{ processDetail.preSaleIds }}</span>
        <span v-else>
          <el-select
          ...
          >
            <el-option
              ...
              :label="item.label"
              :value="item.value"
              :disabled="selectedPresaleList.indexOf(item.value) > -1"
            >
            </el-option>
          </el-select>
        </span>
      ...
      <div class="two">
        <span
          ><span class="require">*</span>预留项目售前人工费用合计（元）</span
        >
        <span v-if="type !== 'add'">{{
          $pubfunc.transformPrice(processDetail.reserveLaborCost, true)
        }}</span>
        <span
          v-else
      ...
      <div class="two">
        <span><span class="require">*</span>预计售前工作人天（人天）</span>
        <span v-if="type !== 'add'">{{
          $pubfunc.transformPrice(processDetail.reserveLaborDays, true)
        }}</span>
        <span
          v-else
          style="padding-left: 0; position: relative"
      ...
      <div class="two">
        <span><span class="require">*</span>预计售前人工明细</span>
        <span
          v-if="type !== 'add'"
          v-html="processDetail.reserveLaborDetail"
        ></span>
        <span
          style="padding-left: 0; position: relative"
          v-else
      ...
      <div class="two">
        <span
          ><span class="require">*</span>预留项目售前差旅费用合计（元）</span
        >
        <span v-if="type !== 'add'">{{
          $pubfunc.transformPrice(processDetail.reserveTravelExpense, true)
        }}</span>
        <span
          style="padding-left: 0; position: relative"
          v-else
      ...
      <div class="two">
        <span><span class="require">*</span>预计售前差旅费用明细</span>
        <span v-if="type !== 'add'">{{
          $pubfunc.transformPrice(processDetail.reserveTravelDetail, true)
        }}</span>
        <span
          style="padding-left: 0; position: relative"
          v-else
  ...
  methods: {
    ...
    formChange(key) {
      if (key !== "preSaleIds") {
        ...
        if (
          key !== "reserveLaborDetail" &&
          key !== "reserveTravelDetail" &&
          key !== "projDesc"
        ) {
